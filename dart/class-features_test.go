...
func TestFindSequenceAndLeadingText(t *testing.T) {
	tests := []struct {
		features    string
		sequence    string
		leadingText string
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("test #%v", i), func(t *testing.T) {
			gotSequence, gotLeadingText := findSequenceAndLeadingText(tt.features)

			if gotSequence != tt.sequence || gotLeadingText != tt.leadingText {
				t.Errorf("findSequenceAndLeadingText(`%v`) =\n(%q,%q)\nwant:\n(%q,%q)",
					tt.features, gotSequence, gotLeadingText, tt.sequence, tt.leadingText)
			}
		})
	}
}
